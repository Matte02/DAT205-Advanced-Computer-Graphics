...
void LodManager::Update(const vec3& CameraPos)
{
	labhelper::perf::Scope s("Update Lod");
	UpdateLodMapPass1(CameraPos);
	UpdateLodMapPass2(CameraPos);
}


void LodManager::UpdateLodMapPass1(const vec3& CameraPos)
{
	labhelper::perf::Scope s("Update Lod Map Pass 1");
	...
	for (int LodMapZ = 0; LodMapZ < m_numPatchesZ; LodMapZ++) {
		for (int LodMapX = 0; LodMapX < m_numPatchesX; LodMapX++) {
			...
			vec3 PatchCenter = vec3(x * (float)m_worldScale, 0.0f, z * (float)m_worldScale);

			float DistanceToCamera = distance(CameraPos, PatchCenter);
...
void LodManager::UpdateLodMapPass2(const vec3& CameraPos)
{
	labhelper::perf::Scope s("Update Lod Map Pass 2");
